...
class WobblySource:
    ...
    def __init__(
        ...
        self.timecode_path = (
            resolve_path(timecode_output_path) if timecode_output_path else None
        )
    ...
    def process(self) -> vs.VideoNode:
        ...
        try:
            ...
            custom_list_processor = CustomListProcessor(
                ProcessPosition.POST_FIELD_MATCH
            )
            src, self.frame_props, self.frame_mapping = custom_list_processor.process(
                src, self.project, self.frame_props, self.frame_mapping, self.presets
            )
            ...
            src, self.frame_props, self.frame_mapping = (
                special_frames_processor.process(
                    src,
                    self.project,
                    self.frame_props,
                    self.frame_mapping,
            ...
            if isinstance(e, WobblyError):
                ...
                raise WobblyProcessError(
                    "Error processing Wobbly project", stage="processing", cause=e
                )
