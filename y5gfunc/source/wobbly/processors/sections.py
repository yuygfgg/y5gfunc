    ...
    def process(
        ...
        with safe_processing("sections_processing"):
            ...
            sorted_sections = sorted(
                sections_list, key=lambda s: s.get(Keys.sections.start, 0)
            )

            # Mark each frame with its section
            self._mark_section_frames(
                clip, sorted_sections, frame_props, frame_mapping, Keys
            )
            ...
            for i, section_info in enumerate(sorted_sections):
                ...
                next_start = (
                    sorted_sections[i + 1].get(Keys.sections.start, clip.num_frames)
                    if i + 1 < len(sorted_sections)
                    else clip.num_frames
                )
