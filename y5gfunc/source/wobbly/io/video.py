    ...
    try:
        if source_filter == "bs.VideoSource":
            ...
            if len(filter_parts) < 2:
                raise WobblyInputError(
                    f"Invalid source filter: {source_filter}, expected format: plugin.function"
                )
        ...
        raise WobblyProcessError(
            f"Failed to load video: {e}", stage="video_loading", cause=e
        )
...
def query_format(
    ...
    return core.query_video_format(
        clip.format.color_family, sample_type.value, bits, subsampling_w, subsampling_h
    ).id


def apply_frame_properties(
    clip: vs.VideoNode, frame_props: FramePropertyMap
) -> vs.VideoNode:
    """
