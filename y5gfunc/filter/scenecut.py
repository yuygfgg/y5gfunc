...
def scd_koala(
    ...
    def _histogram_correlation(hist1: np.ndarray, hist2: np.ndarray) -> float:
        ...
        denominator = np.sqrt(
            np.sum((hist1 - x_mean) ** 2) * np.sum((hist2 - y_mean) ** 2)
        )
    ...
    resized_clip = core.resize2.Bicubic(
        clip, width=256, height=256, format=vs.RGB24, dither_type="none"
    )
    ...
    prev_edge_clip = core.std.Splice(
        [combined_edges[0], combined_edges[:-1]], mismatch=True
    )
    ssim_result = muvsfunc.SSIM(prev_edge_clip, combined_edges)

    def _get_score(
        curr_array: np.ndarray, prev_array: np.ndarray, delta_edges: float
    ) -> float:
        prev_hist = []
        curr_hist = []
        ...
        for c in range(3):
            ...
            hist, _ = (
                np.histogram(channel_data[mask], bins=254, range=(1, 255))
                if np.any(mask)
                else (np.zeros(254), None)
            )
        ...
        delta_histogram = _histogram_correlation(
            np.array(prev_hist), np.array(curr_hist)
        )

        return (
            4.61480465 * delta_histogram + 3.75211168 * delta_edges - 5.485968377115124
        )
    ...
    def _scenecut_eval(n: int, f: list[vs.VideoFrame]) -> vs.VideoFrame:
        ...
        if n >= window_size and current_filtered_score < float(filter_size) / float(
            filter_size + 1
        ):
    ...
    for i in range(window_size + filter_size + 1):
        ...
        shifted = (
            core.std.Splice(
                [core.std.BlankClip(resized_clip, length=abs(rel_idx)), resized_clip],
                mismatch=True,
            )
            if rel_idx < 0
            else resized_clip[rel_idx:]
        )
    ...
    for i in range(window_size + filter_size):
        ...
        shifted = (
            core.std.Splice(
                [core.std.BlankClip(ssim_result, length=abs(rel_idx)), ssim_result],
                mismatch=True,
            )
            if rel_idx < 0
            else ssim_result[rel_idx:]
        )
        prop_clips.append(shifted)
