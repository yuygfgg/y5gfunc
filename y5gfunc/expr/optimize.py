...
def tokenize_expr(expr: str) -> List[str]:
    ...
    expr = re.sub(r'(\w+)\[\s*(-?\d+)\s*,\s*(-?\d+)\s*\](?::(?:c|m))?', r'\1[\2,\3]', expr)
...
def parse_numeric(token: str) -> Union[int, float]:
    ...
    if re.match(r'^0x', token):  # Hexadecimal
        ...
        if '.' in token or 'p' in token.lower():
            parts = re.match(r'^(0x[0-9A-Fa-f]+)(?:\.([0-9A-Fa-f]+))?(?:p([+\-]?\d+))?$', token.lower())
            if parts:
                integer_part = int(parts.group(1), 16)
                fractional_part = 0
                if parts.group(2):
                    fractional_part = int(parts.group(2), 16) / (16 ** len(parts.group(2)))
                exponent = 0
                if parts.group(3):
...
def calculate_binary(op: str, a: Union[int, float], b: Union[int, float]) -> Optional[Union[int, float]]:
...
def calculate_ternary(cond: Union[int, float], true_val: Union[int, float], false_val: Union[int, float]) -> Union[int, float]:
...
def fold_constants(expr: str) -> str:
    ...
    while i < len(tokens):
        ...
        if token in {'exp', 'log', 'sqrt', 'sin', 'cos', 'abs', 'not', 'bitnot', 'trunc', 'round', 'floor'}:
            if len(stack) < 1:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Unary operator requires 1 operand.")
        ...
        if token in {'+', '-', '*', '/', 'max', 'min', 'pow', '**', '>', '<', '=', '>=', '<=', 'and', 'or', 'xor', '%', 'bitand', 'bitor', 'bitxor'}:
            if len(stack) < 2:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Binary operator requires 2 operands.")
            
            if isinstance(stack[-1], (int, float)) and isinstance(stack[-2], (int, float)):
                b = stack.pop()
                a = stack.pop()
        ...
        if token == '?':
            if len(stack) < 3:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Ternary operator requires 3 operands.")
        ...
        if token in {'clamp', 'clip'}:
            if len(stack) < 3:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Clamp operator requires 3 operands.")
        ...
        if token.startswith('swap'):
            ...
            if len(stack) <= n:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Swap requires at least {n+1} items on stack.")
        ...
        if token.startswith('dup'):
            ...
            if len(stack) <= n:
                raise ValueError(f"Stack underflow at token '{token}' at position {i}. Dup requires at least {n+1} items on stack.")
...
def convert_dynamic_to_static(expr: str) -> str:
    ...
    while i < len(tokens):
        ...
        if token.endswith('[]') and len(stack) >= 2:
            ...
            if (x_token_idx >= 0 and is_numeric(result_tokens[x_token_idx]) and 
                y_token_idx >= 0 and is_numeric(result_tokens[y_token_idx])):
...
def update_stack_for_token(token: str, stack: List[Any]) -> List[Any]:
    ...
    if token in {'exp', 'log', 'sqrt', 'sin', 'cos', 'abs', 'not', 'bitnot', 'trunc', 'round', 'floor'}:
        if len(stack) < 1:
            raise ValueError(f"Stack underflow. Unary operator '{token}' requires 1 operand.")
        stack.pop()
        stack.append(None)  # Result placeholder
        
    # Binary operators
    elif token in {'+', '-', '*', '/', 'max', 'min', 'pow', '**', '>', '<', '=', '>=', '<=', 'and', 'or', 'xor', '%', 'bitand', 'bitor', 'bitxor'}:
        if len(stack) < 2:
            raise ValueError(f"Stack underflow. Binary operator '{token}' requires 2 operands.")
        stack.pop()
        stack.pop()
        stack.append(None)  # Result placeholder
    ...
    # swap operations
        ...
        if len(stack) <= n:
            raise ValueError(f"Stack underflow. {token} requires at least {n+1} items on stack.")
    ...
    # drop operations
        ...
        if len(stack) < n:
            raise ValueError(f"Stack underflow. {token} requires at least {n} items on stack.")
    ...
    # Variable operations
        ...
        if token.endswith('!'):  # Store
            if len(stack) < 1:
                raise ValueError("Stack underflow. Variable store requires 1 value on stack.")
            stack.pop()
        else:  # Load
            stack.append(None)
